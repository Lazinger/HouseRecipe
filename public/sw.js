/* service worker : met le carnet en cache pour un fonctionnement hors-ligne.
   Incrémenter CACHE_NAME force le renouvellement du cache au prochain déploiement. */
const CACHE_NAME = "carnet-cache-v92";
const SUPABASE_ORIGIN = "https://bmotbwubruvsrflaufis.supabase.co";
const APP_SHELL = [
  "./",
  "./index.html",
  "./style.css",
  "./manifest.json",
  "./js/main.js",
  "./js/core/dom.js",
  "./js/core/ui.js",
  "./js/data/recipes-data.js",
  "./js/recipes/recipes-store.js",
  "./js/planning/cart.js",
  "./js/planning/fridge.js",
  "./js/recipes/timer.js",
  "./js/recipes/grid.js",
  "./js/recipes/detail.js",
  "./js/recipes/add-form.js",
  "./js/recipes/dyn-rows.js",
  "./js/recipes/scan-recipe.js",
  "./js/recipes/import-url.js",
  "./js/planning/meal-plan.js",
  "./js/data/season-data.js",
  "./js/season/season.js",
  "./js/photos/photo-editor.js",
  "./js/photos/photos.js",
  "./js/recipes/quantity.js",
  "./js/core/icons.js",
  "./js/core/utils.js",
  "./js/auth/supabase-client.js",
  "./js/auth/auth.js",
  "./js/auth/profile.js",
  "./js/recipes/sync.js",
  "./js/core/write-queue.js",
  "./fonts/caveat.woff2",
  "./fonts/dm-sans.woff2",
  "./fonts/fraunces.woff2",
  "./fonts/fraunces-italic.woff2",
  "./icons/icon.svg"
];

self.addEventListener("install", (event) => {
  event.waitUntil(caches.open(CACHE_NAME).then(cache => cache.addAll(APP_SHELL)));
  self.skipWaiting();
});

self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(keys.filter(k => k !== CACHE_NAME).map(k => caches.delete(k))))
  );
  self.clients.claim();
});

self.addEventListener("fetch", (event) => {
  if (event.request.method !== "GET") return;
  /* Les appels à l'API Supabase (auth, données) ne doivent jamais être mis
     en cache : ce sont des réponses dynamiques, pas des fichiers statiques
     du site. Les laisser passer sans interception garde toujours les
     données à jour (ex. getUser() après une mise à jour de profil). */
  if (event.request.url.startsWith(SUPABASE_ORIGIN)) return;
  event.respondWith(
    caches.match(event.request).then(cached => {
      if (cached) return cached;
      return fetch(event.request).then(response => {
        const copy = response.clone();
        caches.open(CACHE_NAME).then(cache => cache.put(event.request, copy));
        return response;
      }).catch(() => cached);
    })
  );
});
